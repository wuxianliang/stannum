...
- [x] WI-12: step 12 — phase 2: BM25F bounds + WAND on LSG4. DONE 2026-09-25 (orchestrator-verified: workspace 582 green, pg18 49/49, pg_test 178, clippy -D warnings clean): selected-fields bounds wired into top_k; **RFC §5.4 min_len* formula corrected via formal erratum** (Σ form unsound as a floor → min form; witness unit test); prune rate recorded (LSG3 112/1088 vs LSG4 362/838 scored/pruned @ LIMIT 10 of 1200); search() multi-column accepted with snippet=>'none' degraded mode; operator scans now scope the scorer/projection from the scan key (scoped_scan_text) and top_k restricts to the scoped candidate set; FIXED Private::from_list field-slot bug (multi-column count scans returned 0).
  - [x] WI-13: step 13 — phase 3: same-field phrases, highlight overload + field fixture, fuzz/oracle field dimension, docs; release 0.4.0 SQL/version. DONE 2026-09-25 (four commits: per-field span solving + heap-corpus field alignment; field-aware highlights/search snippets + the 5-arg `stannum.highlight` overloads; `ranked_fuzz --fields` + `oracle --fields`, which found and fixed two phase-1 field-aware write-buffer/bitmap-path gaps; 0.4.0 version/snapshot/upgrade SQL + upgrade-test field fixture + docs). Gates: workspace 14 suites green, pg18 unit 49/49, pg_test 183/183, clippy -D warnings clean, fmt clean, release install + extension_upgrade (4 paths, fingerprints equal, LSG3 index still read), ranked_fuzz default + --fields seeds 105/206, postings_lifecycle, oracle --fields 16/16 live.
- [x] WI-14: final gates + commit + push. DONE 2026-09-25: orchestrator-verified (workspace 0 failures, pg18 49/49, extension_upgrade green) and pushed 067a557..18bdd8c.

**0.4.0 WAVE COMPLETE 2026-09-25.** All four P0 releases implemented + pushed: 0.2.0 (jieba governance), 0.3.0 (search + observability + pgembed_stannum), 0.4.0 (LSG4 multi-column BM25F). Follow-up DONE 2026-09-25: pgembed STANNUM_COMMIT bumped 7f58bbe → ad4d3b7 (0.4.0) + full bundle rebuilt; `StannumIndex.create(field_weights=...)` unlocked (client-side 2-16-column / f32-parsable positive-weight validation, single-column DDL byte-unchanged, weights = the mapping's columns in DDL order) with 7 new tests + a multi-column notebook section; stale extversion premise in test_stannum_jieba.py updated; pgembed suite 241 passed / 2 skipped; pushed pgembed 785aa58 (pin) + e21ef0e (feature).

**Packaging phase 2 (standalone `pgembed-stannum` wheel) DONE-in-repo 2026-09-25:** `tools/build_standalone_extension_wheel.py` stages the package outside the source tree (library at the package root where `get_extension_path` looks, control + SQL under the package-local pginstall where `get_extension_share_path` looks) and builds a platform-tagged wheel (`pip wheel --build-option=--plat-name`, tag derived from the built library's `vtool` minos); promoted manifest (readme + package-data for all suffixes) + package README; 6 new tests (staging layout vs. the helper contract, missing-prefix/not-staged failures, fixture wheel install resolving the staged artifacts, real-bundle wheel matching the rebuilt prefix, standalone discovery under metadata marking stannum not built). Locally verified end to end: real wheel from the rebuilt 0.4.0 prefix audited (system libs only) and installed in a fresh venv resolving its artifacts; pgembed suite 247 passed / 2 skipped. CI `build_standalone_wheels` job added (macOS host build + audit; manylinux_2_28 cibuildwheel for x86_64 + aarch64, stannum-only prefix built in-container) — its first CI run is the remaining unverified surface (no local container to exercise); the workflow edit also invalidates the macOS native cache key, so the next base-wheel run rebuilds the prefix once. Publish wiring (PyPI trusted publisher for `pgembed-stannum` + release-evidence inclusion) remains a user decision. Pushed pgembed 5f2d473 + 3c653f1.
